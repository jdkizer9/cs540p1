...
main(int argc, char *argv[]) {
    ...
    const char *doc = (const char *) mmap(NULL, (size_t)sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if(doc == MAP_FAILED) {
    ...
    if(correct_mode == true) {
        r = parser.parse(doc, (size_t)sb.st_size);
        traverse(r, 0, indent);
        MyVisitor v(indent);
        r->accept(&v);
        delete r;
    } else {
        r = parser.parse(doc, (size_t)sb.st_size); // warm up
        delete r;
